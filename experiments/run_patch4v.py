...
def test_2v_bound(bound,pf,rf,th):
    ...
    max_x = res[1]
    print "bound idstance"
    print bf.getUlpError(max_x[0],bound[0][0])/1e13
    print bf.getUlpError(max_x[1],bound[1][0])/1e13
...
def read_idx_bound_fromxls(exname,i):
    ...
    ori_bound = bench2v.input_domain[idx]
...
def test_in_bound_domain(n_var,i,idx,bound,th):
    ...
    print et,max_err,mean_err,max_x,srate
...
if __name__ == "__main__":
    ...
    table_name = 'experiment_results4v/table_results/experiment_results_total'+str(num)+'.xls'
    folder_name = os.getcwd()
    file_name = folder_name+'/experiment_results4v/repair_results'+str(num)+ '/test'+str(num)+'/'
    # for i in range(1,2):
    exname = 'experiment_results4v/table_results/experiment_results_total'+str(num)+'.xls'
    idx,ori_bound,nrows = read_idx_bound_fromxls(exname,1)
    # manager = Manager()
    # lock = manager.Lock()
    # for i in range(1,nrows):
    ...
    def test_para_f(i):
        idx, ori_bound, nr = read_idx_bound_fromxls(exname, i)
        return test_in_bound_domain(4, i, idx, ori_bound,th)
    id_lst = range(1, nrows)
    print id_lst
    # for j in id_lst:
    #     test_para_f(j)
    ...
    lock = manager.Lock()
    for result in p.imap(test_para_f, id_lst):
        bf.test2vbound2excel(result[0], result[1], result[2], result[3], exname, result[4],result[5], 16)
    p.close()
    p.join()
    # os.system("./unpatch_cmd.sh")
    # sudo_cmd("./make_install.sh > tmp_log")
