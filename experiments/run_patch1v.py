...
def insert_patch_and_recompile(exname,filename):
    ...
    os.system("cp ../benchmarks/GSL_function/eft_files/* "+"../../../../benchmarks/gsl_src/gsl-2.1-repair/specfunc")
    os.chdir(cwd)
...
def test_1v_bound(bound,pf,rf,th):
    ...
    per_good = good_tests/float(test_num)
...
if __name__ == "__main__":
    ...
    table_name = 'experiment_results/table_results/experiment_results_total'+str(num)+'.xls'
    folder_name = os.getcwd()
    file_name = folder_name+'/experiment_results/repair_results'+str(num)+ '/cpatch'+str(num)+'/'
    ...
    res_lst = []
    # for ix in id_lst:
    #     temp_res = test_para_f(ix)
    #     print temp_res
    #     res_lst.append(temp_res)
    # for result in res_lst:
    #     bf.test1vbound2excel(result[0], result[1], result[2], result[3], table_name, result[4],result[5], 16)
