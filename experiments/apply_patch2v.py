...
def insert_patches_debug(exname):
    data = xlrd.open_workbook(exname)
    table = data.sheets()[0]
    for i in range(1,table.nrows):
        print i
        ...
        sys.stdout = f
        print "#include \"patch_of_" + name + ".c\""
        print "int idx = "+ "find_idx_"+name+"("+x+","+y+");"
        print "if(idx!=0){"
        print "     result->val = accuracy_improve_patch_of_" + name + "(" + x + ","+y+",idx);"
        print "     result->err = GSL_DBL_EPSILON * fabs(result->val);"
        print "     return GSL_SUCCESS;"
        print "}"
        sys.stdout = orig_stdout
...
def insert_patch_and_recompile(exname,filename):
    ...
    sudo_cmd("rm tmp_log")
# insert_patch_and_recompile("experiment_resultstay2v1612/table_results/experiment_results_total2.xls","experiment_resultstay2v1612/repair_results2/test2")
...
def read_idx_bound_fromxls(exname,i):
    ...
    ori_bound = bench2v.input_domain[idx-1][0]
...
if __name__ == "__main__":
    ...
    def test_para_f(i):
        ...
        return test_in_bound_domain(2, i, idx, ori_bound,th)
    id_lst = range(1, nrows)
    # print id_lst
    # for j in [16]:
    #     test_para_f(j)
    # for idx in [15,16]:
    #     result = test_para_f(idx)
