...
def Test_id_fun(n_var,idx,rd_seed,num,level,limit_time,th,table_name,lock):
    ...
    res = main4v(rf, pf, level, th,rd_seed, fnm, limit_time, num, password,max_ret,bound,idx,lock)
    return res
        # ln = [0, 3, 2, 1]
        # k = ln[int(level * 10)]
...
if __name__ == "__main__":
    for i in range(1,2):
        ...
        filename = "experiment_results4v/repair_results" + str(num) + "/test" + repr(int(level * 10))
        ...
        f.close()
        filename = "experiment_results4v/table_results/"
        ...
        lock = manager.Lock()
        def para_f(d):
            return Test_id_fun(4, d, rd_seed, num, level, limit_time, np.power(2.0, th), table_name,lock)
