#include "eft_patch.h"
static double array_idx_sf_airy_Ai_deriv_0[2] = {
-4.649478530614417338e+01,
-4.646991359831716295e+01,
};
static double array_cof_float_sf_airy_Ai_deriv_0[1][8] = {
{
...
static double array_cof_err_sf_airy_Ai_deriv_0[1][8] = {
{
...
static double array_point_sf_airy_Ai_deriv_0[1] = {
-4.648237566972975543e+01,
};
static double array_cofidx_sf_airy_Ai_deriv_0[1] = {
...
double accuracy_improve_patch_of_gsl_sf_airy_Ai_deriv_0(double x)
{
 ...
 while((idx>=0)&&(idx<len_glob)){
     if((x>=array_idx_sf_airy_Ai_deriv_0[idx])&&(x<=array_idx_sf_airy_Ai_deriv_0[idx+1])){
         double point = array_point_sf_airy_Ai_deriv_0[idx];
         double res = 0.0;
         int length = (int)array_cofidx_sf_airy_Ai_deriv_0[idx];
         eft_tay1v(array_cof_float_sf_airy_Ai_deriv_0[idx],array_cof_err_sf_airy_Ai_deriv_0[idx],point,x,&res,length);
         return res;
     }
     else if(x<array_idx_sf_airy_Ai_deriv_0[idx]){
         up_idx = idx;
         idx = dw_idx + floor((idx-dw_idx)/2.0);
     }
     else if(x>array_idx_sf_airy_Ai_deriv_0[idx+1]){
         dw_idx = idx;
